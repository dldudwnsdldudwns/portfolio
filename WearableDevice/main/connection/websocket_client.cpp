...
WebSocketClient::WebSocketClient(const Config &config,
                                 std::function<bool()> is_wifi_available)
...
bool WebSocketClient::initialize() {
  ...
  // NOTE: ping_interval_sec는 "초" 단위 필드입니다. 필요 시 ms -> sec로 변환해
  // 사용하세요. ws_cfg.ping_interval_sec   = config_.ping_interval_sec;
...
void WebSocketClient::async_connect() {
  if (!client_) {
    ESP_LOGE(TAG, "Critical error: Tried to connect before WebSocket client "
                  "was initialized");
    return;
  }
  ...
  auto connect_task = [](void *pvParameter) {
    ...
    while (!self->is_wifi_available_()) {
      ESP_LOGI(
          TAG,
          "Waiting for WiFi to be available before connecting to WebSocket...");
  ...
  xTaskCreatePinnedToCore(connect_task, "ws_connect_task", 4096, this, 5,
                          nullptr, 0);
}
...
void WebSocketClient::start_sender_task() {
  if (sender_task_handle_ == nullptr) {
    ...
    xTaskCreatePinnedToCore(&WebSocketClient::sender_task_wrapper,
                            "data_sender", 4096, this, 5, &sender_task_handle_,
                            0);
  }
}
...
bool WebSocketClient::enqueue_binary_data(std::unique_ptr<uint8_t[]> data,
                                          size_t size) {
  if (!send_queue_)
    return false;
...
void WebSocketClient::websocket_event_handler(void *handler_args,
  ...
  auto handle_ws_event = [](void *pvParameter) {
    auto *tuple = static_cast<
        std::tuple<WebSocketClient *, esp_event_base_t, int32_t, void *> *>(
        pvParameter);
    ...
    auto *event_data =
        static_cast<esp_websocket_event_data_t *>(std::get<3>(*tuple));
    ...
    case WEBSOCKET_EVENT_CONNECTED:
      ...
      ESP_LOGI(TAG, "Sending device name to server: %s",
               config->device_name.c_str());
      if (client->send_text(config->device_name)) {
        ESP_LOGI(TAG, "Device name '%s' sent successfully",
                 config->device_name.c_str());
      } else {
        ESP_LOGE(
            TAG,
            "Connection established, but failed to send device name to server");
    ...
    case WEBSOCKET_EVENT_DISCONNECTED:
      if (!client->trying_to_connect_) {
        ...
        ESP_LOGI(TAG,
                 "WebSocket disconnected (type:%s/%d, esp_err:0x%08x, "
                 "stack:0x%08x, cert_flags:0x%08x)",
                 ws_error_type_str(eh.error_type), eh.error_type,
    ...
    case WEBSOCKET_EVENT_DATA:
      ESP_LOGI(TAG, "WebSocket data received: %zu bytes (payload_len: %zu)",
    ...
    case WEBSOCKET_EVENT_ERROR: {
      ...
      ESP_LOGE(TAG,
               "WebSocket error: %s (type:%s/%d, esp_err:0x%08x, stack:0x%08x, "
               "cert_flags:0x%08x)",
               error_message.c_str(), ws_error_type_str(err.error_type),
               err.error_type,
    ...
    if (do_reconnect) {
      ESP_LOGI(TAG, "Attempting to reconnect WebSocket in %.1f seconds...",
  ...
  auto *param =
      new std::tuple<WebSocketClient *, esp_event_base_t, int32_t, void *>(
          wrap_self, wrap_base, wrap_event_id, wrap_event_data);

  xTaskCreatePinnedToCore(handle_ws_event, "ws_event_handler", 4096, param, 5,
                          nullptr, 0);
}
...
void WebSocketClient::sender_task_wrapper(void *param) {
  ...
  while (true) {
    ...
    if (xQueueReceive(self->send_queue_, &binary_data,
                      pdMS_TO_TICKS(config->data_wait_timeout_ms)) == pdTRUE) {
      if (binary_data && self->is_available()) {
        self->async_send_binary(std::move(binary_data->second),
                                binary_data->first);
      }
      delete binary_data;
...
bool WebSocketClient::send_text(std::string text) {
  return esp_websocket_client_send_text(client_, text.c_str(), text.size(),
                                        portMAX_DELAY) > 0;
}

void WebSocketClient::async_send_binary(std::unique_ptr<uint8_t[]> data,
                                        size_t size) {
  ...
  auto send_task = [](void *pvParameter) {
    auto *param =
        static_cast<std::pair<WebSocketClient *, std::vector<uint8_t>> *>(
            pvParameter);
    WebSocketClient *self = param->first;
    const std::vector<uint8_t> &buf = param->second;

    int sent = esp_websocket_client_send_bin(
        self->client_, reinterpret_cast<const char *>(buf.data()), buf.size(),
        portMAX_DELAY);
  ...
  xTaskCreatePinnedToCore(send_task, "ws_send_task", 8192, param, 5, nullptr,
                          0);
}
...
bool WebSocketClient::send_text_now(const std::string &text) {
  ...
  int sent = esp_websocket_client_send_text(client_,
                                            text.c_str(), // 여기!
