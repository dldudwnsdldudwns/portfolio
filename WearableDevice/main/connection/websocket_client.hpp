...
public:
  struct Config {
    ...
    Config(const std::string &device_name, const std::string &server_ip,
           uint16_t server_port)
        : device_name(device_name),
          uri("ws://" + server_ip + ":" + std::to_string(server_port) + "/"),
          max_queue_size(DEFAULT_WS_MAX_QUEUE_SIZE),
          connect_timeout_ms(DEFAULT_WS_CONNECT_TIMEOUT_MS),
          send_timeout_ms(DEFAULT_WS_SEND_TIMEOUT_MS),
          send_interval_ms(DEFAULT_WS_SEND_INTERVAL_MS),
          data_wait_timeout_ms(DEFAULT_WS_QUEUE_TIMEOUT_MS),
          retry_interval_ms(DEFAULT_WS_RETRY_INTERVAL_MS) {}
  };
