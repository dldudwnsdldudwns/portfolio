...
bool WiFiClient::initialize() {
  ...
  std::strncpy(reinterpret_cast<char *>(wifi_config.sta.ssid),
               config_.ssid.c_str(), sizeof(wifi_config.sta.ssid) - 1);
  std::strncpy(reinterpret_cast<char *>(wifi_config.sta.password),
               config_.password.c_str(), sizeof(wifi_config.sta.password) - 1);
...
void WiFiClient::try_connect() {
  ...
  if (!trying_to_connect_) {
    ...
    auto connect_task = [](void *pvParameter) {
      ...
      ESP_LOGI(TAG, "📶 Attempting to connect to WiFi network: %s",
               client->config_.ssid.c_str());
    ...
    xTaskCreatePinnedToCore(connect_task, "wifi_connect_task", 4096, this, 5,
                            NULL, 0);
  }
}
...
void WiFiClient::handle_wifi_event(int32_t event_id, void *event_data) {
  ...
  case WIFI_EVENT_STA_DISCONNECTED: {
    ...
    if (event_data == nullptr) {
      ...
      ESP_LOGI(TAG, "🔄 Will retry WiFi connection in %.1f seconds...",
               config_.retry_interval_ms / 1000.0f);
    ...
    switch (reasonToCategory.find(event->reason) != reasonToCategory.end()
                ? reasonToCategory.at(event->reason)
                : ReasonCategory::ETC) {
    case ReasonCategory::AP:
      log_message = "❓ SSID not found - check if the WiFi ssid is correct";
      retry_interval =
          2500; // No delay for AP not found, retry as soon as possible
      break;
    case ReasonCategory::AUTH:
      log_message = "🔑 Authentication failed during authentification - check "
                    "SSID and password";
      break;
    case ReasonCategory::ENCRYPTION:
      log_message = "🔑 Authentication failed during encryption - check your "
                    "configuration";
      break;
    ...
    default:
      log_message = (event->reason == WIFI_REASON_CONNECTION_FAIL)
                        ? "❌ Temporary connection failure"
                        : "❔ Unspecified disconnect reason";
      retry_interval = 1000; // retry as soon as possible
      break;
    }
    ESP_LOGI(TAG, "📴 WiFi disconnected - Reason: %d (%s)", event->reason,
             log_message);
    ESP_LOGI(TAG, "🔄 Will retry WiFi connection in %.1f seconds...",
             retry_interval / 1000.0f);
  ...
  default:
    ESP_LOGW(TAG, "event_data exists but not handled for event_id %ld",
             event_id);
    break;
  }
}
