...
bool WebSocketClient::initialize()
{
	...
	ws_cfg.ping_interval_sec             = config_.connect_timeout_ms;
...
void WebSocketClient::async_connect()
{
	...
	{
		ESP_LOGE(TAG,
		         "🛑 Critical error: Tried to connect before WebSocket client was properly initialized or activated");
		return;
	}
	...
	if (!trying_to_connect_)
	{
		...
		xTaskCreatePinnedToCore(connect_task, "ws_connect_task", 4096, this, 5, NULL, 0);
	}
}
...
bool WebSocketClient::enqueue_binary_data(std::unique_ptr<uint8_t[]> data, size_t size)
{
	...
	{
		ESP_LOGW(TAG, "⚠️  Data queue full, dropping oldest data");
		BinaryData* old = nullptr;
		xQueueReceive(send_queue_, &old, 0);
		delete old;
	}
...
void WebSocketClient::websocket_event_handler(void* handler_args, esp_event_base_t wrap_base, int32_t wrap_event_id,
                                              void* wrap_event_data)
{
	auto handle_ws_event = [](void* pvParameter)
	{
		...
		case WEBSOCKET_EVENT_CONNECTED:
			...
			break; // end of case WEBSOCKET_EVENT_CONNECTED
		case WEBSOCKET_EVENT_DISCONNECTED:
			if (!client->trying_to_connect_)
			{
				do_reconnect = true;
				ESP_LOGI(TAG, "❌ WebSocket disconnected from server (reason: %d)", event_data->error_handle);
			}
			break; // end of case WEBSOCKET_EVENT_DISCONNECTED
		case WEBSOCKET_EVENT_DATA:
			ESP_LOGI(TAG, "📩 WebSocket data received: %d bytes (payload_len: %d)", event_data->data_len,
			         event_data->payload_len);
			break; // end of case WEBSOCKET_EVENT_DATA
		case WEBSOCKET_EVENT_ERROR:
		{
			...
			case WEBSOCKET_ERROR_TYPE_SERVER_CLOSE:
				...
				break;
			default:
				error_message = "Unknown WebSocket error type (" + std::to_string(err.error_type) + ")";
				break;
			}
			if (client->trying_to_connect_)
			{
			...
			ESP_LOGE(TAG, "🚨 WebSocket error occurred: %s (code: %d)", error_message.c_str(),
			         event_data->error_handle);
			break;
		} // end of case WEBSOCKET_EVENT_ERROR
		default:
			// ESP_LOGW(TAG, "❔ Unhandled WebSocket event (base: %s, event_id: %d)", base, event_id);
		...
		if (do_reconnect)
		{
			ESP_LOGI(TAG, "🔄 Attempting to reconnect WebSocket in %.1f seconds...",
			         config->retry_interval_ms / 1000.0f);
	...
	auto* param     = new std::tuple<WebSocketClient*, esp_event_base_t, int32_t, void*>(wrap_self, wrap_base,
	                                                                                     wrap_event_id, wrap_event_data);
	xTaskCreatePinnedToCore(handle_ws_event, "ws_event_handler", 4096, param, 5, NULL, 0);
}
...
void WebSocketClient::sender_task_wrapper(void* param)
{
	...
	while (true)
	{
		...
		ESP_LOGI(TAG, "⚠️  No data to send, waiting for new data...");
		vTaskDelay(pdMS_TO_TICKS(config->data_wait_timeout_ms));
	}
}
...
void WebSocketClient::async_send_binary(std::unique_ptr<uint8_t[]> data, size_t size)
{
	...
	auto send_task = [](void* pvParameter)
	{
		...
		int sent = esp_websocket_client_send_bin(self->client_, reinterpret_cast<const char*>(buf.data()), buf.size(),
		                                         portMAX_DELAY);
		if (sent != buf.size())
		{
			ESP_LOGW(TAG, "⚠️ Failed to send data via WebSocket (sent: %d, size: %zu)", sent, buf.size());
	...
	xTaskCreatePinnedToCore(send_task, "ws_send_task", 8192, param, 5, NULL, 0);
}
